...
class RebookFlowModal extends ConsumerStatefulWidget {
  final MyBookingModel booking;
  const RebookFlowModal({super.key, required this.booking});

  @override
  ConsumerState<RebookFlowModal> createState() => _RebookFlowModalState();
}

class _RebookFlowModalState extends ConsumerState<RebookFlowModal>
  ...
  // Try to pre-select the previous booking's address by matching full address text.
  AddressModel? _matchedAddress(List<AddressModel> addresses) {
    final prevFull = widget.booking.address.fullAddress;
    if (prevFull.isEmpty) return null;
    try {
      return addresses.firstWhere((a) => a.fullAddress == prevFull);
    } catch (_) {
      return null;
    }
  }
  ...
  Future<void> _onNext(double total) async {
    ...
    final discountAmount =
        selectedCoupon?.calculateDiscount(_grossAmount) ?? 0.0;

    try {
      await ref.read(bookingServiceProvider).rebookBooking(
            items: _items,
            address: _address!,
            date: _date,
            slot: _slot!,
  ...
  Future<void> _applyCoupon() async {
    ...
    try {
      ...
      final err = found.validate(_grossAmount);
      if (err != null) {
  ...
  Widget build(BuildContext context) {
    ...
    final discount = selectedCoupon?.calculateDiscount(_grossAmount) ?? 0.0;
    final total = (_grossAmount - discount).clamp(0.0, double.infinity);
  ...
  Widget _buildStepContent({
    ...
    return switch (_step) {
      ...
      2 =>
        _buildSummaryStep(selectedCoupon: selectedCoupon, discount: discount),
      3 => _buildPaymentStep(),
  ...
  Widget _buildPaymentStep() {
    ...
    final total =
        (_grossAmount - (coupon?.calculateDiscount(_grossAmount) ?? 0.0))
    ...
    return Container(
      ...
      child: Row(
        children: [
          ...
          FilledButton(
            onPressed:
                (canContinue && !_isCreating) ? () => _onNext(total) : null,
            style: FilledButton.styleFrom(
...
class _RebookSummaryCard extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    ...
    final total =
        (baseAmount + taxAmount - discountAmount).clamp(0.0, double.infinity);
    const months = [
      'Jan', 'Feb', 'Mar', 'Apr', 'May', 'Jun',
      'Jul', 'Aug', 'Sep', 'Oct', 'Nov', 'Dec',
    ];
    final dateStr =
        '${date.day} ${months[date.month - 1]} ${date.year} · ${slot.label}';
              ...
              Wrap(
                ...
                children: periodSlots
                    .map((slot) => TimeSlotCard(
                          ...
                          onTap:
                              slot.isAvailable ? () => onSelect(slot) : null,
                        ))
                    .toList(),
              ),
            ],
