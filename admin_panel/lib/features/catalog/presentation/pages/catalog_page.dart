...
class _CatalogPageState extends ConsumerState<CatalogPage> {
      ...
      builder: (_) => CategoryFormDialog(
        onSave: ({
          ...
          if (mounted) {
            ...
            _showAddSubCategoryWizard(name);
          }
        },
      ),
    );
      ...
      builder: (_) => SubCategoryFormDialog(
        ...
        onSave: ({
          ...
          if (mounted) {
            ...
            _showAddServiceWizard(name, categoryId);
          }
        },
      ),
    );
  ...
  void _openEditSubCategory(SubCategory subCategory) {
    ...
    final cat =
        categories.where((c) => c.id == subCategory.categoryId).firstOrNull;
    showDialog(
      context: context,
      barrierDismissible: false,
      builder: (_) => SubCategoryFormDialog(
  ...
  void _openEditService(Service service) {
    ...
    final subCategories =
        ref.read(subCategoriesNotifierProvider).valueOrNull ?? [];
    final sub =
        subCategories.where((s) => s.id == service.subCategoryId).firstOrNull;
    final cat =
        categories.where((c) => c.id == service.categoryId).firstOrNull;
    showDialog(
      context: context,
      barrierDismissible: false,
      builder: (_) => ServiceFormDialog(
  ...
  void _showAddSubCategoryWizard(String categoryName) {
    showDialog(
      ...
      builder: (ctx) => AlertDialog(
        ...
        content: Text(
            'Would you like to add a Sub Category under "$categoryName"?'),
        actions: [
          TextButton(
            onPressed: () => Navigator.of(ctx).pop(),
          ...
          FilledButton(
            onPressed: () {
              ...
              final cats =
                  ref.read(categoriesNotifierProvider).valueOrNull ?? [];
              final cat =
                  cats.where((c) => c.name == categoryName).firstOrNull;
              if (cat == null) return;
  ...
  void _showAddServiceWizard(String subCategoryName, String categoryId) {
    showDialog(
      context: context,
      builder: (ctx) => AlertDialog(
        shape:
        ...
        content: Text(
            'Would you like to add a Service under "$subCategoryName"?'),
        actions: [
          TextButton(
            onPressed: () => Navigator.of(ctx).pop(),
            child: const Text('Not Now'),
  ...
  Widget build(BuildContext context) {
    ...
    final callbacks = CatalogCallbacks(
      ...
      onAddSubCategory: (cat) => _openCreateSubCategory(
            categoryId: cat.id,
            categoryName: cat.name,
          ),
