-- Migration: Fix dispatch timeout recording in booking_assignments
...
BEGIN
  ...
  IF v_candidate.vendor_id IS NOT NULL THEN
    -- Mark any previous pending assignment records as timed_out at their exact timeout time
...
-- Fix existing booking_assignments timed_out rows where responded_at exceeds configured timeout
DO $$
DECLARE
  v_tier_timeout INT := 60;
BEGIN
  SELECT COALESCE(tier_timeout_seconds, 60) INTO v_tier_timeout
  FROM public.dispatch_settings LIMIT 1;

  UPDATE public.booking_assignments
  SET responded_at = assigned_at + (v_tier_timeout || ' seconds')::INTERVAL
  WHERE status = 'timed_out'
    AND (responded_at IS NULL OR responded_at > assigned_at + (v_tier_timeout || ' seconds')::INTERVAL);
END;
$$;
