-- ─────────────────────────────────────────────────────────────────────────────
-- Refund Eligibility: block re-submission when a refund has already been
-- completed for the booking.
--
-- Updates customer_create_refund_request (from migration 000016) to add a
-- guard that checks refund_transactions for a completed row on this booking.
-- Rejected/failed-without-completion tickets are unaffected — customers can
-- still submit a new request for those bookings.
-- ─────────────────────────────────────────────────────────────────────────────
...
BEGIN
  ...
  -- Checks refund_transactions directly so that rejected/failed-without-
  -- completion tickets do not block a new submission.
  IF EXISTS (
    SELECT 1
