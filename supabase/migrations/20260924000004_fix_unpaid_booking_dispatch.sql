-- ─────────────────────────────────────────────────────────────────────────────
-- Fix: add payment-status early exit to dispatch_booking_next_tier.
--
-- Root cause: the RPC had no payment_status check.  When called for an
-- unpaid online booking, it reached the UPDATE that triggers
-- trg_guard_razorpay_assignment, which raised an EXCEPTION.  That exception
-- propagated out of the RPC as an unhandled PostgreSQL error rather than a
-- clean JSON response, giving the admin a cryptic failure instead of a
-- clear explanation.
--
-- This migration adds an explicit early-exit BEFORE any INSERT or UPDATE so
-- that unpaid online bookings return a clean { success: false,
-- reason: "payment_unverified" } JSON, while preserving the DB-level trigger
-- as a final safety net for any direct UPDATE path.
--
-- All other logic is identical to migration 20260805000003.
-- ─────────────────────────────────────────────────────────────────────────────
...
BEGIN
  ...
  -- Abort if booking is already accepted, in_progress, completed.
  IF v_booking.status IN ('accepted', 'in_progress', 'completed', 'awaiting_verification') THEN
    RETURN jsonb_build_object(
      'success', false, 'reason', 'already_accepted', 'status', v_booking.status
    );
  END IF;

  -- Strictly abort if not in auto-dispatch flow.
  IF v_booking.dispatch_status IS NOT NULL AND v_booking.dispatch_status != 'dispatching' THEN
    RETURN jsonb_build_object(
      'success', false,
      'reason', 'manual_assignment',
      'message', 'Booking is manually assigned or not in auto-dispatch flow'
    );
  END IF;

  -- Block dispatch of unverified online-payment bookings.
  -- Mirrors the condition in trg_guard_razorpay_assignment but returns a
  -- clean JSON error instead of raising a PostgreSQL exception.
  IF LOWER(COALESCE(v_booking.payment_method, '')) IN ('razorpay', 'online')
     AND COALESCE(v_booking.payment_status, '') <> 'success'
  THEN
    RETURN jsonb_build_object(
      'success',  false,
      'reason',   'payment_unverified',
      'message',  'Cannot dispatch booking '
                    || p_booking_id::TEXT
                    || ' — online payment has not been verified (payment_status='
                    || COALESCE(v_booking.payment_status, 'null')
                    || '). Verify payment or cancel the booking first.'
    );
  END IF;
  ...
  IF v_candidate.vendor_id IS NOT NULL THEN
    -- Mark any previous pending assignments as timed_out.
    ...
    INSERT INTO public.booking_assignments (
      booking_id, vendor_id, tier_id, tier_priority, attempt_number, status, assigned_at
    ) VALUES (
      p_booking_id, v_candidate.vendor_id, v_candidate.tier_id,
      v_candidate.tier_priority, v_attempt_count + 1, 'pending', now()
    ) RETURNING id INTO v_assignment_id;

    -- Update booking status.
    UPDATE public.bookings
       SET vendor_id                  = v_candidate.vendor_id,
           status                     = 'assigned',
    ...
    INSERT INTO public.notifications (
      ...
      'vendor', v_candidate.vendor_id,
      'New Booking Request',
      'New booking request #' || COALESCE(v_booking.booking_number, '') || '. Tap to review.',
      'vendor_assigned', 'booking', p_booking_id, false, now()
    );
