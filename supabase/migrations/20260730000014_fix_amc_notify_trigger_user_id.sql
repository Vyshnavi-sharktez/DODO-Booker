-- Fix: notifications.user_id is a UUID column; the string 'system' written by
-- both AMC scheduling request triggers is invalid. Replace with NULL.
-- Admin notifications are broadcast (no specific recipient) so NULL is correct
-- and consistent with the customer app's broadcast notification pattern.

-- ── Re-create: INSERT trigger (new request submitted) ─────────────────────────
...
-- ── Re-create: UPDATE trigger (customer cancels request) ──────────────────────
